...
class AccountCheckDeposit(models.Model):
    ...
    def _compute_check_deposit(self):
        ...
        mapped_data = {}
        for x in rg_res:
            check_deposit_id = x.get("check_deposit_id", [False])[0]
            debit = x.get("debit", 0)
            amount_currency = x.get("amount_currency", 0)
            count = x.get("check_deposit_id_count", 0)

            mapped_data[check_deposit_id] = {
                "debit": debit,
    ...
    def _compute_in_hand_check_account_id(self):
        for rec in self:
            account = rec.journal_id.filtered(
                lambda line: line.inbound_payment_method_line_ids.payment_method_id.code == "manual"
            ).default_account_id
    ...
    def get_all_checks(self):
        ...
        all_pending_checks = self.env["account.move.line"].search(
            [
                ...
                ("default_account_id", "=", self.in_hand_check_account_id.id),
                ("payment_id", "!=", False),
                ("debit", ">", 0),

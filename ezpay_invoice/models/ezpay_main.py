...
class EzpayInvoice():
    ...
    def __init__(self):
        ...
        self.Send['Print'] = EzpayPrintMark.No
        self.Send['Donation'] = EzpayDonation.No
        self.Send['LoveCode'] = ''
        ...
        self.Send['InvType'] = ''
        self.Send['vat'] = EzpayVatType.Yes
        self.Send['DelayFlag'] = ''
    ...
    def Check_Out(self):
        ...
        return Ezpay_Invoice_Send.CheckOut(arParameters, self.HashKey, self.HashIV, self.Invoice_Method,
                                           self.Invoice_Url)
...
class Ezpay_Invoice_Send():
    ...
    def CheckOut(arParameters=dict, HashKey='', HashIV='', Invoice_Method='', ServiceURL=''):
        ...
        arParameters = Ezpay_Invoice_Send.process_send(arParameters, HashKey, HashIV, Invoice_Method, ServiceURL)

        szResult = Ezpay_IO.ServerPost(arParameters, ServiceURL)
        # 回傳資訊處理
        arParameters_Return = Ezpay_Invoice_Send.process_return(szResult, HashKey, HashIV, Invoice_Method)
    ...
    def process_send(arParameters=dict, HashKey='', HashIV='', Invoice_Method='', ServiceURL=''):
        ...
        arParameters = Ezpay_Invoice_Send.urlencode_process(arParameters, InvoiceObj.urlencode_field)
        # 5欄位例外處理方式(送壓碼前)
        arException = InvoiceObj.check_exception(arParameters)
        # 6產生壓碼
        arParameters['CheckMacValue'] = Ezpay_Invoice_Send.generate_checkmacvalue(arException,
    ...
    def process_return(sParameters='', HashKey='', HashIV='', Invoice_Method=''):
        ...
        if 'CheckMacValue' in arParameters:
            CheckMacValue = Ezpay_Invoice_Send.generate_checkmacvalue(arException, InvoiceObj_Return.none_verification,
                                                                      HashKey, HashIV)
            if CheckMacValue != arParameters['CheckMacValue']:
                ...
                print('注意：壓碼錯誤')
        # 10處理需要urldecode的參數
        arParameters = Ezpay_Invoice_Send.urldecode_process(arParameters, InvoiceObj_Return.urlencode_field)
        return arParameters
    ...
    def urlencode_process(arParameters=dict, urlencode_field=dict):
        for key, val in arParameters.items():
            if key in urlencode_field:
                ...
                arParameters[key] = Ezpay_CheckMacValue.do_str_replace(arParameters[key])
        return arParameters
    ...
    *6, 9產生壓碼
    ...
    def generate_checkmacvalue(arParameters=dict, none_verification=dict, HashKey='', HashIV=''):
        ...
        sCheck_MacValue = Ezpay_CheckMacValue.generate(sub_parameters, HashKey, HashIV, ECPay_EncryptType.ENC_MD5)
    ...
    *10處理urldecode的參數
    ...
    def urldecode_process(arParameters=dict, urlencode_field=dict):
        for key, val in arParameters.items():
            if key in urlencode_field:
                arParameters[key] = Ezpay_CheckMacValue.restore_str_replace(arParameters[key])
...
class Ezpay_AllowanceByCollegiate():
    # 所需參數
    parameters = dict({
        'TimeStamp': '',
        'MerchantID': '',
